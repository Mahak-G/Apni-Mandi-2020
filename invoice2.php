...
if(!empty($_COOKIE['payment']))
        echo $_COOKIE["prod"];
	//cookie of product names
	$cookie = $_COOKIE['details'];
$cookie = stripslashes($cookie);
$productdetails = json_decode($cookie, true);
print_r($productdetails);
//cookie of product quantity
//cookie of product names
	$cookiee = $_COOKIE['quantity'];
$cookiee = stripslashes($cookiee);
$productquantity = json_decode($cookiee, true);
print_r($productquantity);
       ?>
		...
		<?php
			...
			//extracting hours from datetime and giving slots
			$sql2 = "SELECT EXTRACT(HOURS FROM DATETIME) AS HOURS";
					if($stmt2 = mysqli_prepare($link, $sql2)){
						mysqli_stmt_bind_param($stmt2, "sssiss");
