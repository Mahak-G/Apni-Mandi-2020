...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	else
    {
        ...
        if($stmt = mysqli_prepare($link, $sql))
		{
				...
				if(mysqli_stmt_execute($stmt))
				{
					...
					{
					  $USERID_err = "This USERID is not taken.";
	       			}
					else
					{
	...
	{
        $PSWD_er = "Please enter your PSWD.";
	}
	else
	{
	...
	if(empty($USERID_er) && empty($PSWD_er))
	{
        
        $sql = "SELECT USERID,PSWD FROM USERS WHERE USERID = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql))
		{
			...
			if(mysqli_stmt_execute($stmt))
			{
                ...
                // Check if USERID exists, if yes then verify PSWD

				if(mysqli_stmt_num_rows($stmt) >= 1)
				{                    
                    
					mysqli_stmt_bind_result($stmt, $USERID, $PSWD);
                    ...
                    if(mysqli_stmt_fetch($stmt))
					{
						...
						if($res1!=false)
						{
							...
							{
                            // PSWD is correct, so start a new session
							session_start();
							...
							{
                            // Display an eror message if PSWD is not valid
                            $PSWD_er = "The PSWD you entered was not valid.";
                        	}
						   }
						}
						else
						{  
							$USERID_er = "Something not right";
						 }
					  }
					}
  			     }
			...
			<div class="w3_login_module">
				<div class="module form-module">
				  ...
				  <div class="form">
					...
					<form method="post">
					  <input type="text" name="USERID" placeholder="Username" required=" "><span><?php echo $USERID_er; ?></span>
					  <input type="password" name="password" placeholder="Password" required=" "><span><?php echo $PSWD_er; ?></span>
					  <input type="submit" value="Login" name="login">
					</form>
				  </div>
				  <div class="cta"> 
					<p><ins><a href="signup.php">Create account</a></ins></p>
					<p><ins><a href="resetpassword.php">Forgot your password?</a></ins></p>
				  </div>
			</div>
		</div>
			<script>
