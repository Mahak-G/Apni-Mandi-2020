...
$USERID_err = $PSWD_err = $cPSWD_err = $CONTACT_err = $EMAIL_err = $CITY_err = $VILLAGE_err=$AADHAR_err="";
...
WHERE `USERID` = ?";
    ...
    {
        $PSWD_err = "Please enter a PSWD.";     
    } 
    elseif(strlen(trim($_POST["PSWD"])) < 6)
    {
        $PSWD_err = "PSWD must have atleast 6 characters.";
    } 
    else
    {
    ...
    // Validate confirm PSWD
    if(empty(trim($_POST["cPSWD"])))
    {
        $cPSWD_err = "Please confirm PSWD.";     
    } 
     else
    {
        $cPSWD = trim($_POST["cPSWD"]);
        if(empty($PSWD_err) && ($PSWD != $cPSWD))
	{
            $cPSWD_err = "PSWD did not match.";
        }
    }
    ...
    {
        $VILLAGE_err = "Please enter a VILLAGE.";     
    } 
    else
    {
   ...
   if(empty(trim($_POST["CONTACT"])))
    ...
    {
        $CONTACT_err = "CONTACT number must have 10 characters.";
    } else
	...
	if(empty($USERID_err) && empty($PSWD_err) && empty($cPSWD_err) && empty($CONTACT_err)
	 && empty($CITY_err)&& empty($VILLAGE_err) && empty($EMAIL_err))
    {
		$_SESSION["USERID"] = $USERID;
        
		$sql = "INSERT INTO USERS (USERID, PSWD, ROLE, CONTACT, EMAIL, CITY,VILLAGE) VALUES (?, ?, ?, ?,?, ?,?)";
					
         
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt, "sssisss", $param_USERID, $param_PSWD,$param_ROLE,
						$param_CONTACT,$param_EMAIL,$param_CITY,$param_VILLAGE);
            $param_USERID = $USERID;
			$param_PSWD = $PSWD;
			$param_ROLE = $ROLE;
		...
		$param_VILLAGE = $VILLAGE;
            if(mysqli_stmt_execute($stmt)){
				...
				if($ROLE=="Farmer")
				{
    				...
    				{
     				   $AADHAR_err = "Please enter an AADHAR.";     
   					 } else
   					 {
				...
				<div class="module form-module">
				  <div class="toggle"><i class="fa fa-times fa-pencil"></i>
				   ...
				   <div class="form">
					...
					<form method="post">
					  <input type="text" name="USERID" placeholder="Username"  required=" "><span><?php echo $USERID_err; ?></span>
					 ...
					 <input type="password" name="cPSWD" placeholder="Confirm Password" value="<?php echo $cPSWD; ?>" required=" "><span><?php echo $cPSWD_err; ?></span>
					  <input type="text" name="CITY"  placeholder="City" required=" "><span><?php echo $CITY_err; ?></span>
					  <input type="text" name="VILLAGE"  placeholder="Village" required=" "><span><?php echo $VILLAGE_err; ?></span>
					  <input type="email" name="EMAIL" placeholder="Email Address"  value="<?php echo $EMAIL; ?>" required=" "><span><?php echo $EMAIL_err; ?></span>
					  <input type="text" name="CONTACT" placeholder="Phone Number" value="<?php echo $CONTACT; ?>" required=" "> <span><?php echo $CONTACT_err; ?></span>
					...
					<?php
					if(!empty($_SESSION["role"]) && $_SESSION["role"] == "Farmer"){
					echo "<form method='post'>
					  <input type='text' name='AADHAR' placeholder='XXXX XXXX XXXX'  required=' '><span><?php echo $AADHAR_err; ?></span>
					  <input type='submit' value='Register'>
					</form>";
					}
					?>
