...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	session_start(); 
					...
					if(empty(trim($_POST["AADHAR"])))
    				{
					 ...
					 $m = "UPDATE verification set contact='1' where aadhar='$AADHAR'";
					 if($mq = mysqli_prepare($link, $m))
					 {
						 if(mysqli_stmt_execute($mq))
						 {
							$cv=mysqli_stmt_affected_rows($mq);
							 if($cv!=0)
							 {
								$up= "UPDATE verification set contact='2' where aadhar='$AADHAR'";
								$zx = mysqli_prepare($link, $up);
								$er=mysqli_stmt_execute($zx);
								mysqli_stmt_close($zx);
								$sql = "UPDATE USERS set aadhar='$AADHAR' where PSWD='$US'";
				...
				<div class="module form-module">
				  <div class="toggle"><i class="fa fa-times fa-pencil"></i>
				   ...
				   <div class="form">
					...
					echo $a;
					?>
					  <input type="text" name="AADHAR" placeholder="aadhar"  required=" "><span><?php echo $AADHAR_err; ?></span>
